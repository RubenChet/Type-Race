<template>
  <div
    class="flex justify-center flex-wrap m-7 overflow-hidden w-full h-14"
    ref="flexWrapContainer"
  >
    <div
    ...
    >
      <p
        ...
        v-bind:class="{
          ...
          cursor_next:
            idx === word.length - 1 && idx === currentLetter - 1 && index === word_index,
        }"
  ...
  methods: {
      ...
      if (e.key === "Backspace") {
        ...
        if (this.game.words[this.word_index] === this.words_list_copy[this.word_index]) {
          ...
          if (this.word_index > 0) {
            if (
              this.word_is_valid[this.word_index - 1] == false &&
          ...
          this.game.words[this.word_index] = this.game.words[this.word_index].substring(
            0,
            this.game.words[this.word_index].length - 1
          );
          this.currentLetter--;
    ...
    letter_listener(elem) {
      ...
      if (typedLetter == "Delete") {
        ...
        if (typedLetter === " ") {
          const w_status = this.check_letter_value(
            this.game.letters[this.word_index],
            this.word_index
          );
          if (w_status == true) {
